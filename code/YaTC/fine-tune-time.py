...
def main(args):
    ...
    for repeat_num in range(5):
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            val_time_list[repeat_num] = val_time_list[repeat_num] + end_time_val - start_time_val
            ...
            log_stats = {**{f'train_{k}': v if not isinstance(v, np.ndarray) else v.tolist() for k, v in train_stats.items()},
                        **{f'val_{k}': v if not isinstance(v, np.ndarray) else v.tolist() for k, v in val_stats.items()},
                        'epoch': epoch,
        ...
        train_time_list.append(end_time_train - start_time_train)
        ...
        if args.test_path is not None:
            ...
            test_time_list.append(end_time_test - start_time_test)
            inference_time_list.append(test_stats["infer_time"])
            with open(f"results/{args.dataset}/{args.data_path[-1]}_{args.blr}_{args.frozen}_results.json", "a") as results_file:        
                json.dump(test_stats, results_file)
                results_file.write("\n")
        
        print(f"Train time: {train_time_list}")
        print(f"Val time: {val_time_list}")
        print(f"Test time: {test_time_list}")
