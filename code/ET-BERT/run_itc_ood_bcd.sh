...
cd /root/Repository/Traffic/code/ET-BERT || exit 1

PY=/root/.local/share/mamba/envs/etbert/bin/python
DATASET=ITC-Net-Blend-60
LOG_DIR=logs
RESULT_DIR=results/${DATASET}
RUN_LOG=${LOG_DIR}/itc_ood_bcd_runner.log

mkdir -p "${LOG_DIR}" "${RESULT_DIR}"

echo "[$(date -Is)] BCD runner started" >> "${RUN_LOG}"
...
for F in B C D; do
  ...
  if [[ -f "${RESULT_FILE}" ]] && grep -q '"epoch": "test"' "${RESULT_FILE}"; then
    echo "[$(date -Is)] fold_${F} test result found" >> "${RUN_LOG}"
  else
    echo "[$(date -Is)] fold_${F} test result missing; continuing to next fold" >> "${RUN_LOG}"
  fi
done

echo "[$(date -Is)] BCD runner finished" >> "${RUN_LOG}"
