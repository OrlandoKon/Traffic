...
from split_pcap import split_pcap_to_packets_parallel, split_pcap_to_sessions_parallel, process_directory_parallel
from split_pcap import split_pcap_to_packets_with_limited
...
def split_cap(pcap_path, pcap_file, pcap_name, pcap_label='', dataset_level = 'flow'):
    ...
    if dataset_level == 'flow':
        split_pcap_to_sessions_parallel(pcap_file, output_path)
    elif dataset_level == 'packet':
        # split_pcap_to_packets_parallel(pcap_file, output_path)
        split_pcap_to_packets_with_limited(pcap_file, output_path, 10000)
...
def generation(pcap_path, samples, features, splitcap = False, payload_length = 128, payload_packet = 5, dataset_save_path = "I:/ex_results/", dataset_level = "flow"):
    ...
    for key in tqdm.tqdm(session_pcap_path.keys()):
        ...
        if dataset_level == "flow":
            ...
            if splitcap:# not splitcap
                ...
                print(session_pcap_path[key])
                # TCP : 144 bytes = 24 + N * (16 + 104) UDP 102 bytes = 24 + N * (16 + 62)
                # process_directory_parallel(session_pcap_path[key], {'TCP': 0.14, 'UDP': 0.1})
                process_directory_parallel(session_pcap_path[key], {'TCP': 0.14, 'UDP': 0.1})
            if label_id[key] not in dataset:
                dataset[label_id[key]] = {
                    "samples": 0,
    ...
    # split two stages: one is split pcap, the other is generate
    if splitcap:
        return
